...
import (
	...
	"iot_pj/internal/hummingbird/core/captcha"
...
func (uapp *userApp) UserRegister(ctx context.Context, req dtos.RegisterRequest) error {
    // 验证验证码
    captchaSvc := captcha.New(uapp.lc)
    if err := captchaSvc.VerifyCaptcha(req.CaptchaKey, req.Captcha, req.Email); err != nil {
        uapp.lc.Errorf("验证码验证失败: %v", err)
        return err
    }
    ...
    if err != nil {
        return errort.NewCommonEdgeX(500, "密码加密失败", err)
    }
    ...
    // 调用数据库接口
    if _, err := uapp.dbClient.AddUser(newUser); err != nil {
        uapp.lc.Errorf("用户注册失败: %v", err)
        return errort.NewCommonEdgeX(500, "数据库写入失败", err)
    }
    return nil
}
