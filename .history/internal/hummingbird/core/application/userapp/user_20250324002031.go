...
func (uapp *userApp) UserRegister(ctx context.Context, req dtos.RegisterResponse) error {
    // 密码加密处理
    hashedPassword, err := bcrypt.GenerateFromPassword([]byte(req.pas), bcrypt.DefaultCost)
    if err != nil {
        return err
    }
    ...
    // 调用数据库接口
    _, err = uapp.dbClient.AddUser(newUser)
    if err != nil {
