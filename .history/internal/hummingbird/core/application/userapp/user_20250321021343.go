...
func (uapp *userApp) Register(ctx context.Context, req dtos.RegisterRequest) error {
	// 验证验证码
	localCache, _ := uapp.getCache()
	storedCode, found := localCache.Get(req.Email)
	if !found || storedCode != req.VerifyCode {
		return errort.NewCommonErr(
			errort.ErrCodeVerifyCode,
			fmt.Errorf("验证码无效或已过期"),
		)
	}

	// 检查用户名唯一性
	if exists, _ := uapp.repo.UserExists(ctx, req.Username); exists {
		return errort.NewCommonErr(
			errort.ErrCodeUsernameExist,
			fmt.Errorf("用户名 %s 已存在", req.Username),
		)
	}
	...
	if err := uapp.repo.CreateUser(ctx, user); err != nil {
		return errort.Wrap(err, "创建用户失败")
	}

	// 发送欢迎邮件
	if err := uapp.emailService.SendWelcomeEmail(req.Email); err != nil {
		return errort.NewCommonErr(
			errort.ErrCodeSendEmail,
			fmt.Errorf("发送欢迎邮件失败: %w", err),
		)
	}
