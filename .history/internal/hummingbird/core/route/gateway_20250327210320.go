...
func RegisterGateway(engine *gin.Engine, dic *di.Container) {
	...
	v1.POST("auth/captcha", func(c *gin.Context) {
		var req struct {
		...
		lc := logger.NewClient("captcha-service", "info", "")
		captchaSvc := captcha.New(lc)
		key, err := captchaSvc.SendEmailCaptcha(c.Request.Context(), req.Email)
		if err != nil {
			c.JSON(500, gin.H{"success": false, "message": "发送验证码失败"})
			return
		}
