...
import (
	"iot_pj/internal/hummingbird/core/controller/http/gateway"
	"iot_pj/internal/hummingbird/core/controller/http/websocket"
	"iot_pj/internal/pkg/di"
	"iot_pj/internal/tools/jwt"
...
func RegisterGateway(engine *gin.Engine, dic *di.Container) {
	ctl := gateway.New(dic)
	aiCtl := gateway.NewAIController(dic)
	...
	/*******AI功能 *******/
	{
		v1Auth.GET("ai/recommendations", aiCtl.GetAIRecommendations)
		v1Auth.GET("ai/recommendations/:id", aiCtl.GetAIRecommendationByID)
		v1Auth.POST("ai/recommendations", aiCtl.CreateAIRecommendation)
		v1Auth.POST("ai/recommendations/generate", aiCtl.GenerateAIRecommendation)
		v1Auth.GET("ai/qa/history", aiCtl.GetAIQAHistory)
		v1Auth.POST("ai/qa", aiCtl.CreateAIQA)
	}
	/*******首页 *******/
	{
