...
// @Accept json
// @Produce json
// @Param request body dtos.RegisterRequest true "注册参数"
// @Success 200 {object} response.Response
// @Router /api/v1/user/register [post]
func (ctl *UserController) Register(c *gin.Context) {
	var req dtos.RegisterRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		ctl.Fail(c, errort.NewCommonErr(
			errort.ErrCodeInvalidParams,
			fmt.Errorf("无效请求参数: %w", err),
		))
		return
	}
	...
	if err := ctl.getUserApp().Register(c.Request.Context(), req); err != nil {
		ctl.Fail(c, err)
		return
	}
