...
import { Property } from '@/types/history'
import {
  Table,
  Card,
  Tabs,
...
// Used in handleSingleQuery and handleCompareQuery
...
export default function HistoryQueryPage () {
    ...
    try {
      ...
      if (result.success && result.data) {
        setProperties((prev) => {
          const newProps = {
            ...prev,
            [deviceId]: result.data as Property[],
          }
          return newProps
        })
      }
  ...
  const handleSingleQuery = async () => {
    ...
    setLoading(true)
    try {
      const result: QueryResult<DataPoint[]> = await queryDevicePropertyHistory(selectedDevice, selectedProperty, startTime, endTime)
  ...
  const handleCompareQuery = async () => {
    ...
    try {
      ...
      const result: QueryResult<Record<string, DataPoint[]>> = await queryMultipleDeviceProperties(queries, compareStartTime, compareEndTime)
      ...
      if (result.success) {
        ...
        Object.entries(resultData).forEach(([label, series], index) => {
          ...
          // 为表格创建行
          (series as DataPoint[]).forEach((point: DataPoint) => {
            tableRows.push({
        ...
        alert(`查询失败: ${result?.message || result?.error || '未知错误'}`)
      }
    } catch (error) {
                ...
                <div>
                  ...
                  <div className="border rounded-md overflow-auto max-h-[400px]">
                    ...
                    >
                      ...
                      <Table.Column title="设备" dataIndex="device" key="device" />
                      <Table.Column title="属性" dataIndex="property" key="property" />
