...
export default function RegisterPage () {
  ...
  const onFinish = async (values: any) => {
    ...
    if (!allValues.captchaKey) {
      console.error('CaptchaKey missing in form values')
      message.error('验证码KEY缺失，请重新获取验证码')
      return
    }
    ...
    const requestData = {
      ...
      captcha: allValues.captcha,
      captchaKey: allValues.captchaKey,
    }
    console.log('Register request payload:', requestData)

    try {
      const resp = await Register(requestData)
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      if (response?.success) {
                        // 确保设置 captchaKey 字段
                        form.setFieldsValue({
                          captchaKey: response.result?.key || '', // 确保 captchaKey 被正确设置
                          captcha: '',
                        })
                        console.log('验证码 key 已设置:', response.result?.key)
                        message.success({
