...
export default function RegisterPage () {
  ...
  const onFinish = async (values: any) => {
    console.log('Register form values:', JSON.stringify(values, null, 2))

    // 验证验证码必填
    // if (!values?.captchaKey || !values?.captcha) {
    //   message.error('请先获取并输入验证码')
    //   return
    // }
    // if (!values?.captcha) {
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      if (response?.success) {
                        ...
                        message.success({
                          content: '验证码已发送到您的邮箱，10分钟内有效', // 保持与后端一致的过期时间
                          key: 'captcha',
                          duration: 5,
                        })
                      } else {
